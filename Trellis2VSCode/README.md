...
#### Running on This Machine (WSL2 / Ubuntu)

The conda environment is named `trellis2vscode`. Use the following command to run from the repo root:

```sh
source ~/miniconda3/etc/profile.d/conda.sh && conda activate trellis2vscode && python3 example_ag.py
```

**Notes:**
- Do NOT use `python` (not in PATH) — use `python3`
- Do NOT use `conda run -n trellis2vscode` — `conda` is not in PATH by default in non-interactive shells; source the init script first as shown above
- The working directory must be the repo root (`Trellis2VSCode/`) so that relative paths like `assets/hdri/forest.exr` resolve correctly

**`example_ag.py` config variables** (edit at the top of the file):
```python
folder = "/home/ag/AI_Objects_Linux/Trellis2VSCode/assets/ag_examples/<output_folder>"
image_file = "<image_filename>.webp"   # image must be inside `folder`
```

Outputs saved to `folder`:
- `output.mp4` — PBR preview video
